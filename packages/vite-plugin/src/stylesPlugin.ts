...
import { cacheDir, writeStyles } from '@amotify/loader-shared'

import type { PluginOption, ViteDevServer } from 'vite'
import type { Options } from '@amotify/loader-shared'
import type { PluginContext } from 'rollup'

const debug = _debug('amotify:styles')
...
const styleImportRegexp = /(@use |meta\.load-css\()['"](amotify(?:\/lib)?\/styles(?:\/main(?:\.sass)?)?)['"]/

export function stylesPlugin (options: Options): PluginOption {
    const amotifyBase = path.dirname(require.resolve('amotify/package.json'))
    ...
    async function awaitBlocking () {
        ...
        timeout = setTimeout(() => {
            console.error('amotify:styles fallback timeout hit', {
    ...
    return {
        ...
        async resolveId (source, importer, custom) {
            if (
                importer &&
                source.endsWith('.css') &&
                isSubdir(amotifyBase, path.isAbsolute(source) ? source : importer)
            ) {
        ...
        async transform (code, id) {
            if (
                ...
                return code.replace(styleImportRegexp, '$1".cache/amotify/styles.scss"')
            }
        },
        load (id) {
